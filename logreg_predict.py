...
if __name__ == "__main__":
    ...
    try:
        if len(sys.argv) != 3:
            raise Exception("\n  Args list:\n\tFirst_arg:DataSetTest Path.\n\tSecond_arg:Generated predictData.json Path.")
        if not sys.argv[1].endswith(".csv"):
            raise Exception(f"The file '{sys.argv[1]}' must be a CSV file.")
        if not sys.argv[2].endswith("predictData.json"):
            raise Exception(f"The file '{sys.argv[2]}' must be generated and it s name: predictData.json")
        if not os.path.exists(sys.argv[1]):
            raise Exception(f"The file '{sys.argv[1]}' does not exist.")
        if not os.path.exists(sys.argv[2]):
            raise Exception(f"The file '{sys.argv[2]}' does not exist.")
